...
A431 <- readMSnSet2(file = csvfile, ecol = 2:11, skip = 0, fnames = 1)
...
proteinnames <- key[rownames(A431)[rownames(A431) %in% key[, 1]], 2] # careful in wrong order
...
experiment <- new("MIAPE",
                  ...
                  samples = list(
                    species = "Human epidermoid carcinoma",
                    operator = "Lukas Orre"
                  ),
...
fd$protein <- NA
fd$protein[rownames(A431) %in% key[, 1]] <- proteinnames
...
## Normalise
orre2019a431 <- normalise(orre2019a431, method = "sum")

## Map cluster to organelles (manually extracted)
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust1"] <- c("Golgi, Endo/Lyosome")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust2"] <- c("ER, peroxisome")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust3"] <- c("ER, Mitochondrion")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust4"] <- c("Plasma Membrane")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust5"] <- c("Nucleosol, Ribosome")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust6"] <- c("Nucleus (speckles)")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust7"] <- c("Nucleus (Nucleolus)")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust8"] <- c("Nucleosol")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust9"] <- c("Cytosol_1")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust10"] <- c("Cytosol_2")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust11"] <- c("Cytosol_3")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust12"] <- c("Cytosol_4")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust13"] <- c("Cytosol_5")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust14"] <- c("Mitochondria matrix")
fData(orre2019a431)[,2][fData(orre2019a431)[,2] == "mclust15"] <- c("Mitochondria membrane")

#plot2D(orre2019a431, main = "A431", method = "t-SNE")
#addLegend(orre2019a431, where = "bottomleft", ncol = 1, cex = 0.9)

## Phenodata
pData(orre2019a431)$fraction <- matrix(unlist(strsplit(sampleNames(orre2019a431), "[.]")), 10, 3, byrow = T)[,1]
pData(orre2019a431)$replicate <- matrix(unlist(strsplit(sampleNames(orre2019a431), "[.]")), 10, 3, byrow = T)[,2]

stopifnot(length(pData(orre2019a431)$replicate) == ncol(e)) # check columns and experiments match


orre2019a431@processingData <- process
stopifnot(validObject(orre2019a431))

save(orre2019a431, file="../../data/orre2019a431.rda",
