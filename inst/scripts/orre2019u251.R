...
U251 <- readMSnSet2(file = csvfile, ecol = 2:11, skip = 0, fnames = 1)
...
proteinnames <- key[rownames(U251)[rownames(U251) %in% key[, 1]], 2] # careful in wrong order
...
## Experiment info
toName <- paste0(colnames(U251)[1:10])
colnames(e) <- toName
...
fd$protein <- NA
fd$protein[rownames(U251) %in% key[, 1]] <- proteinnames
...
## Normalise
orre2019u251 <- normalise(orre2019u251, method = "sum")

## Map cluster to organelles (manually extracted)
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust1"] <- c("Golgi, Endo/Lyosome")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust2"] <- c("ER, peroxisome")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust3"] <- c("ER, Mitochondrion")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust4"] <- c("Plasma Membrane")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust5"] <- c("Nucleosol, Ribosome")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust6"] <- c("Nucleus (speckles)")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust7"] <- c("Nucleus (Nucleolus)")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust8"] <- c("Nucleosol")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust9"] <- c("Cytosol_1")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust10"] <- c("Cytosol_2")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust11"] <- c("Cytosol_3")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust12"] <- c("Cytosol_4")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust13"] <- c("Cytosol_5")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust14"] <- c("Mitochondria matrix")
fData(orre2019u251)[,2][fData(orre2019u251)[,2] == "mclust15"] <- c("Mitochondria membrane")

plot2D(orre2019u251, main = "U251")
addLegend(orre2019u251, where = "bottomleft", ncol = 1, cex = 0.9)

## Phenodata
pData(orre2019u251)$fraction <- matrix(unlist(strsplit(sampleNames(orre2019u251), "[.]")), 10, 3, byrow = T)[,1]
pData(orre2019u251)$replicate <- matrix(unlist(strsplit(sampleNames(orre2019u251), "[.]")), 10, 3, byrow = T)[,2]

stopifnot(length(pData(orre2019u251)$replicate) == ncol(e)) # check columns and experiments match


orre2019u251@processingData <- process
stopifnot(validObject(orre2019u251))

save(orre2019u251, file="../../data/orre2019u251.rda",
