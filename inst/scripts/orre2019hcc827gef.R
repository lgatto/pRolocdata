...
csvfile <- "../../inst/extdata/HCC827-gef.csv"
...
HCC827Gef <- readMSnSet2(file = csvfile, ecol = 2:11, skip = 0, fnames = 1)
...
proteinnames <- key[rownames(HCC827Gef)[rownames(HCC827Gef) %in% key[, 1]], 2] # careful in wrong order
...
## Experiment info
toName <- paste0(colnames(HCC827Gef)[1:10])
colnames(e) <- toName
...
fd$protein <- NA
fd$protein[rownames(HCC827Gef) %in% key[, 1]] <- proteinnames
...
## Normalise
orre2019hcc827gef <- normalise(orre2019hcc827gef, method = "sum")

## Map cluster to organelles (manually extracted)
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust1"] <- c("Golgi, Endo/Lyosome")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust2"] <- c("ER, peroxisome")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust3"] <- c("ER, Mitochondrion")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust4"] <- c("Plasma Membrane")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust5"] <- c("Nucleosol, Ribosome")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust6"] <- c("Nucleus (speckles)")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust7"] <- c("Nucleus (Nucleolus)")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust8"] <- c("Nucleosol")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust9"] <- c("Cytosol_1")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust10"] <- c("Cytosol_2")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust11"] <- c("Cytosol_3")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust12"] <- c("Cytosol_4")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust13"] <- c("Cytosol_5")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust14"] <- c("Mitochondria matrix")
fData(orre2019hcc827gef)[,2][fData(orre2019hcc827gef)[,2] == "mclust15"] <- c("Mitochondria membrane")

plot2D(orre2019hcc827gef, main = "HCC827Gef")
addLegend(orre2019hcc827gef, where = "bottomleft", ncol = 1, cex = 0.9)

## Phenodata
pData(orre2019hcc827gef)$fraction <- matrix(unlist(strsplit(sampleNames(orre2019hcc827gef), "[.]")), 10, 4, byrow = T)[,1]
pData(orre2019hcc827gef)$replicate <- matrix(unlist(strsplit(sampleNames(orre2019hcc827gef), "[.]")), 10, 4, byrow = T)[,2]

stopifnot(length(pData(orre2019hcc827gef)$replicate) == ncol(e)) # check columns and experiments match


orre2019hcc827gef@processingData <- process
stopifnot(validObject(orre2019hcc827gef))

save(orre2019hcc827gef, file="../../data/orre2019hcc827gef.rda",
